...
const char *CLASS_EXCEPTION = "java/lang/ClassNotFoundException";
...
void printPacket(u_char *user, u_char *cp, u_int caplen, u_int length,
	...
	fprintf(stderr, "Packet: u = %s, l = %d of %d, t = %u.%06u, d = ", user,
			caplen, length, seconds, useconds);
...
Java_net_sourceforge_jpcap_capture_PacketCapture_findDevice(JNIEnv *env,
		jobject obj) {
...
Java_net_sourceforge_jpcap_capture_PacketCapture_open(JNIEnv *env,
		jobject object, jint instance, jstring jdevice, jint snaplen,
	...
	PD[instance] = pcap_open_live((char*) device, snaplen, promiscuous, timeout,
			ebuf);
 ...
 */
JNIEXPORT void JNICALL
Java_net_sourceforge_jpcap_capture_PacketCapture_openOffline(JNIEnv *env,
		jobject object, jint instance, jstring jfileName) {
	const char* fileName;
	char ebuf[PCAP_ERRBUF_SIZE];
	jfieldID fid;
	jclass cls;
 ...
 */
JNIEXPORT void JNICALL
Java_net_sourceforge_jpcap_capture_PacketCapture_endCapture(JNIEnv *env,
		jobject obj, jint instance) {
	attr[instance].pcapGo = FALSE;
}
...
Java_net_sourceforge_jpcap_capture_PacketCapture_getNetwork(JNIEnv *env,
		jobject obj, jstring jdevice) {
	char ebuf[PCAP_ERRBUF_SIZE];
	int localnet = 0;
	int netmask = 0;
	...
	if (pcap_lookupnet((char*) device, (bpf_u_int32 *) &localnet,
 ...
 */
JNIEXPORT jint JNICALL
Java_net_sourceforge_jpcap_capture_PacketCapture_getNetmask(JNIEnv *env,
		jobject obj, jstring jdevice) {
	char ebuf[PCAP_ERRBUF_SIZE];
	const char *device = env->GetStringUTFChars(jdevice, 0);
	int localnet = 0;
 ...
 */
JNIEXPORT jint JNICALL
Java_net_sourceforge_jpcap_capture_PacketCapture_getLinkLayerType(JNIEnv *env,
		jobject obj, jint instance) {
 ...
 */
JNIEXPORT void JNICALL
Java_net_sourceforge_jpcap_capture_PacketCapture_setFilter(JNIEnv *env,
		jobject object, jint instance, jstring jfilter, jboolean optimize) {
 ...
 */
JNIEXPORT jint JNICALL
Java_net_sourceforge_jpcap_capture_PacketCapture_getSnapshotLength(JNIEnv *env,
		jobject obj, jint instance) {
 ...
 */
JNIEXPORT void JNICALL
Java_net_sourceforge_jpcap_capture_PacketCapture_close(JNIEnv *env,
		jobject object, jint instance) {
	if (PD[instance] != NULL)
 ...
 */
JNIEXPORT void JNICALL
Java_net_sourceforge_jpcap_capture_PacketCapture_setupStatistics(JNIEnv *env,
		jobject object, jint instance) {
...
Java_net_sourceforge_jpcap_capture_PacketCapture_lookupDevices(JNIEnv *env,
		jclass cls) {
	char ebuf[PCAP_ERRBUF_SIZE];
	...
	char *s;
	for(;ifr < last; s = (char*)ifr + ifrSize, ifr=(struct ifreq *)s) {
	...
	if (total > 0) {
		devices = env->NewObjectArray((jsize) total,
	...
	if (devices == NULL) {
		devices = env->NewObjectArray((jsize) 0,
				env->FindClass("java/lang/String"), NULL);
	}
	return devices;
}

const char *rcsid = "$Id: jpcap.c,v 1.24 2004/10/07 18:04:41 pcharles Exp $";
