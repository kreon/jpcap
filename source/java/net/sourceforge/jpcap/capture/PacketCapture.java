...
/**
 * This class is the core of packet capture in jpcap. It provides a high-level
 * interface for capturing network packets by encapsulating libpcap.
 * <p>
 * If you want to capture network packets, implement PacketListener and register
 * with an instance of this class. When packets arrive, the object will call you
 * back via packetArrived().
 * <p>
 ...
 * <p>
 * PacketCapture utilizes libpcap's pcap_loop(). For pcap_dispatch()-type
 * behavior, see the endCapture() method.
 *
...
public class PacketCapture extends PacketCaptureBase implements PacketCaptureCapable {
	...
	/**
	 * Close cleans up after a packet capture session. It does _not_ terminate a
	 * packet capture. capture() does not return control until 'count' packets
	 * are captured.
	 * <p>
	 * If you are looking for a way to signal an end to a capture session before
	 * a set number of packets are received, check out the class
	 * SyncPacketCapture.
	 */
	 ...
	 */
	/*
	 * public static native String[] lookupDevices() throws
	 * CaptureDeviceLookupException;
	 */

	/**
	 * note: the following code is a viable non-native alternative to
	 * lookupDevices(). However, it requires jdk1.4. Currently, all of the jpcap
	 * code builds on jdk1.2, making it 'portable' to a large number of
	 * platforms and vm's.
	 *
	 */
	...
	/**
	 * The packet capture library sets up the statistic counter members when
	...
	private String _rcsid = "$Id: PacketCapture.java,v 1.18 2004/10/22 21:28:04 pcharles Exp $";
}
